...
def acoustic_loss(batch: dict, mel, mel_post, v) -> dict[str, torch.Tensor]:
    ...
    losses = {
        ...
        "pitch": masked_mean(F.mse_loss(v.pitch, batch["pitch"], reduction="none"), batch["mel_mask"]),
        "energy": masked_mean(F.mse_loss(v.energy, batch["energy"], reduction="none"), batch["mel_mask"]),
    }
