...
class FastSpeech2(nn.Module):
    ...
    def forward(self, ids, id_mask, durations=None, pitch=None, energy=None, pitch_scale: float = 1.0, rate: float = 1.0,
        ...
        v = self.variance(x, id_mask, durations, pitch, energy, pitch_scale=pitch_scale, rate=rate,
    ...
    @torch.no_grad()
    def synthesize(self, ids: torch.Tensor, pitch_scale: float = 1.0, words=None, word_ids=None, rate: float = 1.0) -> torch.Tensor:
        ...
        _, mel, _ = self(ids, mask, pitch_scale=pitch_scale, words=words, word_ids=word_ids, rate=rate)
        return mel
